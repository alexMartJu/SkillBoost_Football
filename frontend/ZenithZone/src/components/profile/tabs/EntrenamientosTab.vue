  ...
  <div class="entrenamientos-tab p-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
      ...
      <div class="btn-group" role="group">
        <button type="button" class="btn btn-outline-primary" :class="{ 'active': activeFilter === 'activos' }"
          @click="setFilter('activos')">
          Activos
        </button>
        <button type="button" class="btn btn-outline-primary" :class="{ 'active': activeFilter === 'expirados' }"
        ...
        <div class="card h-100 shadow-sm border-0 rounded-3 hover-card">
          ...
          <div class="card-body">
            ...
            <div class="d-flex justify-content-between align-items-center mb-2">
              ...
              <div>
                ...
                <span>{{ formatTime(reserva.entrenamiento.horarioPista?.fechaInicio) }} - {{
                  formatTime(reserva.entrenamiento.horarioPista?.fechaFin) }}</span>
              </div>
            </div>
            <div class="d-flex justify-content-between align-items-center mb-2">
              <div>
                <i class="bi bi-person-fill me-1"></i>
                <span>{{ reserva.entrenamiento.entrenador?.nombre }} {{ reserva.entrenamiento.entrenador?.apellidos
                  }}</span>
              </div>
              <div>
          ...
          <div class="card-footer bg-white border-top-0">
            <div class="d-flex justify-content-between align-items-center">
              ...
              <div>
                <button v-if="activeFilter === 'activos'" @click="cancelarEntrenamiento(reserva.entrenamiento.slug)"
                  class="btn btn-sm btn-outline-danger">
                  <i class="bi bi-x-circle me-1"></i>Cancelar
                </button>
                <evaluation-button 
    v-else-if="reserva.evaluacion"
    :evaluation-id="reserva.evaluacion.id"
    :reserva="reserva"
    @view-evaluation="openEvaluationModal" />
              </div>
            </div>
          </div>
        </div>
    ...
    <!-- Modal de Evaluación de Rendimiento -->
    <evaluacion-rendimiento-modal 
    v-model="showEvaluacionModal"
    :evaluacion="selectedEvaluacion"
    :nombre-entrenamiento="selectedEntrenamiento?.nombre || ''" 
    :loading="loadingEvaluacion" 
  />

    <!-- Toast para notificaciones -->
    <div class="position-fixed bottom-0 end-0 p-3" style="z-index: 11">
      <div id="entrenamientoToast" class="toast" :class="{ 'show': showToast }" role="alert" aria-live="assertive"
        aria-atomic="true">
...
import EvaluationButton from '@/components/shared/buttons/EvaluationButton.vue';
import EvaluacionRendimientoModal from '@/components/profile/EvaluacionRendimientoModal.vue';

export default {
  name: 'EntrenamientosTab',
  components: {
    EvaluationButton,
    EvaluacionRendimientoModal
  },
  setup() {
    ...
    const loadingEvaluacion = ref(false);
    ...
    const openEvaluationModal = (reserva) => {
      loadingEvaluacion.value = true;
      selectedEntrenamiento.value = reserva.entrenamiento;
      selectedEvaluacion.value = reserva.evaluacion;
